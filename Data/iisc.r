...
# Define the start and end dates based on the dates CSV file
special_dates <- dates1$X1
  
if (start %in% special_dates) {
  special_index <- which(special_dates == start)
  special_end_date <- special_dates[special_index + 1]
    
  dateseq1 <- seq(start, to = special_end_date, by = "1 day")
  dateseq2 <- seq(from = special_end_date + 1, by = "1 day", 

  length = length(l) - length(dateseq1))
    
  l2 <- c(dateseq1, dateseq2)
} else {
    l2 <- seq(start, by = "1 day", length = length(l))
}
