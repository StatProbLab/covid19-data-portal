...
#stdercsir= read.csv(../Data/stdercsir.csv")
library(plotly)
library(lubridate)
library(reticulate)
...
#print(typeof(a))
#print(dates1$X1[i])
#print(typeof(dates1$X0[i]))
start=as.Date(dates1$X1[i],"%d/%m/%y")
year(start)<-2020

dist_data[(length(dist_data)+1):length(l)]=-99 #always get 7 more -99
...
whole_data2=subset(pred2,pred2$District==dist_names[i],select=c(Rescaled.Infected,Date))
k=which(whole_data2$Date==start)
print(l2)
print(k)
print(k+length(l2)-1)
...
if(i==6) {whole_data4=subset(pred4,pred4$District==dist_names[i]&pred4$Date%in%l2,select=c(Predicted.Infected,Date))
}else {whole_data4=subset(pred4,pred4$District==dist_names[i],select=c(Predicted.Infected,Date))}
print(pred4_data)
k=which(whole_data4$Date==start)
print(k)
...
{
if(i==6){whole_data5=subset(pred5,pred5$District==dist_names[i]&pred5$d%in%l2,select=c(Infected,Date))
}else {whole_data5=subset(pred5,pred5$District==dist_names[i],select=c(Infected,Date))}
k=which(whole_data5$Date==start)
...
k=which(whole_data5$Date=="2020-04-19")
k=which(whole_data5$Date==start)
pred5_data=whole_data5$Infected[k:(k+length(l2)-1)]
...
#print(start)
#print(whole_data5)
#print(subset(pred5,pred5$District==dist_names[i]&pred5$Date%in%l2,select=c(Infected,Date)))
#print(class(l2))
#print(pred5_data)
...
path <- file.path('/opt/lampp/htdocs/covid19-data-portal/Data/Output/plotlyfigures/', "plotlyfigures", name)
