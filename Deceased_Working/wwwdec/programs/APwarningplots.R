###############################################################################
## Title: COVID-19 Data Analysis and Prediction for Andhra Pradesh Districts ##
## Input: earlywarning/DISTRICTINF AP.xlsx	                                 ##
...
# Reading the file
data<- read_excel("earlywarning/DISTRICTINF AP.xlsx")
...
#Name Districts
df1=subset(data,data$DISTRICTCODE=="32")
df2=subset(data,data$DISTRICTCODE=="33")
df3=subset(data,data$DISTRICTCODE=="34")
df4=subset(data,data$DISTRICTCODE=="35")
df5=subset(data,data$DISTRICTCODE=="36")
df6=subset(data,data$DISTRICTCODE=="37")
df7=subset(data,data$DISTRICTCODE=="38")
df8=subset(data,data$DISTRICTCODE=="39")
df9=subset(data,data$DISTRICTCODE=="40")
df10=subset(data,data$DISTRICTCODE=="41")
df11=subset(data,data$DISTRICTCODE=="42")
df12=subset(data,data$DISTRICTCODE=="43")
df13=subset(data,data$DISTRICTCODE=="44")


District<- rep("Ananthapur",length(nrow(df1)))
df1$District <-District

District<- rep("Chittoor",length(nrow(df2)))
df2$District <-District

District<- rep("East Godavari",length(nrow(df3)))
df3$District <-District

District<- rep("Guntur",length(nrow(df4)))
df4$District <-District

District<- rep("Kadapa",length(nrow(df5)))
df5$District <-District

District<- rep("Krishna",length(nrow(df6)))
df6$District <-District

District<- rep("Kurnool",length(nrow(df7)))
df7$District <-District

District<- rep("Nellore",length(nrow(df8)))
df8$District <-District

District<- rep("Prakasam",length(nrow(df9)))
df9$District <-District

District<- rep("Srikakulam",length(nrow(df10)))
df10$District <-District

District<- rep("Vishakapatnam",length(nrow(df11)))
df11$District <-District

District<- rep("Vizianagaram",length(nrow(df12)))
df12$District <-District

District<- rep("West Godavari",length(nrow(df13)))
...
for(j in seq(length(unique_district)))
{
  ...
  #prediction date=3
  df_k2=subset(df_f,df_f$Date>="2020-07-20"&df_f$Date<"2020-09-20")
  ...
  #prediction date=6
  df_k5=subset(df_f,df_f$Date>="2021-04-25"&df_f$Date<"2021-06-10")
