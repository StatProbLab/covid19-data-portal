###############################################################################
## Title: COVID-19 Data Analysis and Prediction for Chattisgarh Districts ##
## Input: earlywarning/DISTRICTINF CG.xlsx	                                 ##
## Output: plots of total active cases over time, estimating future trends   ##
##         for each district.                                                ##
...
data<- read_excel("earlywarning/DISTRICTINF CG.xlsx")
...
#Name Districts
df1=subset(data,data$DISTRICTCODE=="298")
df2=subset(data,data$DISTRICTCODE=="299")
df3=subset(data,data$DISTRICTCODE=="300")
df4=subset(data,data$DISTRICTCODE=="301")
df5=subset(data,data$DISTRICTCODE=="302")
df6=subset(data,data$DISTRICTCODE=="303")
df7=subset(data,data$DISTRICTCODE=="304")
df8=subset(data,data$DISTRICTCODE=="305")
df9=subset(data,data$DISTRICTCODE=="306")
df10=subset(data,data$DISTRICTCODE=="307")
df11=subset(data,data$DISTRICTCODE=="308")
df12=subset(data,data$DISTRICTCODE=="309")
df13=subset(data,data$DISTRICTCODE=="310")
df14=subset(data,data$DISTRICTCODE=="311")
df15=subset(data,data$DISTRICTCODE=="312")
df16=subset(data,data$DISTRICTCODE=="313")
df17=subset(data,data$DISTRICTCODE=="314")
df18=subset(data,data$DISTRICTCODE=="315")
df19=subset(data,data$DISTRICTCODE=="316")
df20=subset(data,data$DISTRICTCODE=="317")
df21=subset(data,data$DISTRICTCODE=="318")
df22=subset(data,data$DISTRICTCODE=="319")
df23=subset(data,data$DISTRICTCODE=="320")
df24=subset(data,data$DISTRICTCODE=="321")
df25=subset(data,data$DISTRICTCODE=="322")
df26=subset(data,data$DISTRICTCODE=="323")
df27=subset(data,data$DISTRICTCODE=="324")
df28=subset(data,data$DISTRICTCODE=="325")
df29=subset(data,data$DISTRICTCODE=="326")


District<- rep("Balod",length(nrow(df1)))
df1$District <-District

District<- rep("Baloda Bazar",length(nrow(df2)))
df2$District <-District

District<- rep("Balrampur",length(nrow(df3)))
df3$District <-District

District<- rep("Bastar",length(nrow(df4)))
df4$District <-District

District<- rep("Bemetara",length(nrow(df5)))
df5$District <-District

District<- rep("Bijapur",length(nrow(df6)))
df6$District <-District

District<- rep("Bilaspur",length(nrow(df7)))
df7$District <-District

District<- rep("Dantewada",length(nrow(df8)))
df8$District <-District

District<- rep("Dhamtari",length(nrow(df9)))
df9$District <-District

District<- rep("Durg",length(nrow(df10)))
df10$District <-District

District<- rep("Gariaband",length(nrow(df11)))
df11$District <-District

District<- rep("Gaurela-Pendra-Marwahi",length(nrow(df12)))
df12$District <-District

District<- rep("Janjgir-Champa",length(nrow(df13)))
df13$District <-District

District<- rep("Jashpur",length(nrow(df14)))
df14$District <-District

District<- rep("Kabirdham",length(nrow(df15)))
df15$District <-District

District<- rep("Kanker",length(nrow(df16)))
df16$District <-District

District<- rep("Kondagaon",length(nrow(df17)))
df17$District <-District

District<- rep("Korba",length(nrow(df18)))
df18$District <-District

District<- rep("Koriya",length(nrow(df19)))
df19$District <-District

District<- rep("Mahasamund",length(nrow(df20)))
df20$District <-District

District<- rep("Mungeli",length(nrow(df21)))
df21$District <-District

District<- rep("Narayanpur",length(nrow(df22)))
df22$District <-District

District<- rep("Raigarh",length(nrow(df23)))
df23$District <-District

District<- rep("Raipur",length(nrow(df24)))
df24$District <-District

District<- rep("Rajnandgaon",length(nrow(df25)))
df25$District <-District

District<- rep("Sukma",length(nrow(df26)))
df26$District <-District

District<- rep("Surajpur",length(nrow(df27)))
df27$District <-District

District<- rep("Surguja",length(nrow(df28)))
df28$District <-District

District<- rep("Others",length(nrow(df29)))
df29$District <-District


df=rbind(df1,df2,df3,df4,df5,df6,df7,df8,df9,df10,df11,df12,df13,df14,df15,df16,df17,df18,df19,df20,df21,df22,df23,df24,df25,df26,df27,df28)
df=df[,c(6,2,3,4,5)]
