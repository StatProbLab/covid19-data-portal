###############################################################
## Title: Statewise Doubling Time for Infection Growth       ##
##        in India                                           ##
...
## Doubling Time graphs for each State
## Required Libraries
library(plotly)
...
dt=function(i){
  ...
  n = filter(allindia,allindia$State==state_names[i])
  n = na.omit(n)
  n_bar=n[,3]
  ...
  ## plotly graph for doubling time of each state- color coded according to the lockdown phase
  ...
  fig %>% layout(title= state_names[i],titlefont=f2, yaxis=list(title="Doubling Time", titlefont=f),xaxis= list(title="Date",tickangle=-45,tickfont=f3))
}

## calling the function dt for each state and saving the graph 
for(i in 1:length(state_names)){
  name = paste(state_names[i],"dt.html", sep="-")
  name2 = paste0("./plotlyfigures/",state_names[i],"-dt.svg")
  fig = dt(i)
  save_image(fig, name2)
