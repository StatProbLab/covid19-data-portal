###############################################################
## Title: EWS for each districts in Karnataka                ##
...
for(j in seq(length(unique_district)))
{
  ...
  #prediction date=1
  df_k=subset(df_f,df_f$Date>="2020-06-20"&df_f$Date<"2020-07-20")
  ...
  df_f4a_month<-data.frame((format(df_h4$Date,"%Y-%m")),df_h4)
  names(df_f4_month) <- c("Month","No.","Date","District","Total Active","Rate","Rate-mu","categ","model_generated")
  
  #prediction date=7
  df_k5a=subset(df_f,df_f$Date>="2021-12-15"&df_f$Date<"2022-02-26")
  df_n5a=subset(df_k5a,df_k5a$categ>0.1)
  df_h5a=subset(df_f,df_f$Date>=as.Date.character(df_n5a[1,2])&df_f$Date<"2022-02-26")
  #estimate lamda(t)
  ...
  names(df_f5a_month) <- c("Month","No.","Date","District","Total Active","Rate","Rate-mu","categ","model_generated")

  #prediction date=8
  df_k5b=subset(df_f,df_f$Date>="2021-12-31"&df_f$Date<"2022-02-26")
  df_n5b=subset(df_k5b,df_k5b$categ>0.1)
  df_h5b=subset(df_f,df_f$Date>=as.Date.character(df_n5b[1,2])&df_f$Date<"2022-02-26")
  #estimate lamda(t)
  
  for(i in seq(unique(nrow(df_h5b))))
  {
    if (i == 1)
      df_h5b$model[i]=df_h5b$`Total Active`[1]
    if(i>1)
      df_h5b$model[i]<-df_h5b$model[i-1]*(1+(df_h5b$categ[1]-0.1))
  }
  
  df_h5b$model=floor(df_h5b$model)
  
  
  df_f5b_month<-data.frame((format(df_h5b$Date,"%Y-%m")),df_h5b)
  names(df_f5b_month) <- c("Month","No.","Date","District","Total Active","Rate","Rate-mu","categ","model_generated")
  
    #prediction date=9
  df_k5XXX=subset(df_f,df_f$Date>="2022-1-15"&df_f$Date<"2022-02-26")
  df_n5XXX=subset(df_k5XXX,df_k5XXX$categ>0.1)
  df_h5XXX=subset(df_f,df_f$Date>=as.Date.character(df_n5XXX[1,2])&df_f$Date<"2022-02-26")
  #estimate lamda(t)
  ...
  names(df_f5XXX_month) <- c("Month","No.","Date","District","Total Active","Rate","Rate-mu","categ","model_generated")
  ...
  total_predict=ggplot()+
    ...
    scale_x_date(date_breaks = "1 month",date_labels = "%Y-%m")+scale_color_viridis(discrete = TRUE) + xlab("Date") + ylab("count") + ggtitle(head1) + theme_minimal() + theme(plot.title = element_text(color = "#D55E00",face = "bold", size = 40), axis.title = element_text(color = "#D55E00", size = 34), axis.text.y = element_text(size = 28),axis.text.x = element_text(angle=45,hjust = 1,size = 28))+  
    geom_line(data = df_f5b_month, aes(x =Date,y=model_generated), color = "red",size=1)+
    scale_x_date(date_breaks = "1 month",date_labels = "%Y-%m")+scale_color_viridis(discrete = TRUE) + xlab("Date") + ylab("count") + ggtitle(head1) + theme_minimal() + theme(plot.title = element_text(color = "#D55E00",face = "bold", size = 40), axis.title = element_text(color = "#D55E00", size = 34), axis.text.y = element_text(size = 28),axis.text.x = element_text(angle=45,hjust = 1,size = 28))+  
