###############################################################
## Title: Districtwise Doubling Time for Infection Growth    ##
##        in Karnataka                                       ##
...
a=length(allka$Date)
d=seq(as.Date("22-03-20","%d-%m-%y"),length=a,by="1 day") ## date sequence
...
## function calculate doubling time for each state and plotly graph for the same
dt=function(i){
  ##Calculating doubling time for state i.
  ##Algorithm explained in doubling time notes at: https://www.isibang.ac.in/~athreya/incovid19/doublingnote.pdf
  n= filter(allka,District==district_names[i])
  n = na.omit(n)
  n_bar=n[,3]
  start=max(min(which(n_bar>50)),min(which(n_bar>2*n_bar[1])))
  date1=1:length(n_bar)
  f1<-approxfun(date1,n_bar,method="linear")
  yj=n_bar
  y2=n_bar[start:length(n_bar)] 
  dtime=c() 
  
  ## finding the time at which half of the count was acheived and storing them in the variable dtime
  ...
  ## Making table that stored the doubling time for all districts
  cat=rep(0,length(dtime))
  df=data.frame("Date"=n$Date[c(1:length(dtime))],"Doubling Time"=dtime,"Cat"=cat)
  alldf=rbind(alldf,cbind(rep(dis[i,1],length(df$Cat)),df))
  write.csv(alldf,"AllDistrictsDt.csv")
  ...
  ## plotly graph for doubling time of each district- color coded according to the lockdown phase
  ...
  fig %>% layout(title= dis[i,1],titlefont=f2, yaxis=list(title="Doubling Time", titlefont=f),xaxis= list(title="Date",tickangle=-45,tickfont=f3))
}

## calling the function dt for each district with cases above the cutoff and saving the graphs
for(i in 1:30){
  name = paste(i,"dt.html", sep="-")
  name2=paste0("./plotlyfigures/",i,"-dt.svg")
  save_image(dt(i),name2)
  path <- file.path(getwd(), "plotlyfigures", name)
  htmlwidgets::saveWidget(dt(i), file=path, selfcontained = FALSE, libdir = "plotly.html")
}s