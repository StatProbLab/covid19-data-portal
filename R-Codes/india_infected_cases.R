############################################################
## Title: Total Infected Covid19 Cases in India           ##
...
# plot of total infected cases in India over time
a_ = ggplot(region, aes(x=Date, y=Infected)) +
  labs(
    title=paste0("Total Coronavirus Cases in ",region_name),
    x="Date", y="Infected Cases"
  ) +
...
an_ = animate(a_+transition_reveal(region$Date), renderer = gifski_renderer(paste0('./graphs/infected_cases_',region_name,'.gif')),
